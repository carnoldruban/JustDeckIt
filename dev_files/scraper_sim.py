...
class Scraper:
    def __init__(self, data_queue, test_file="blackjack_progressive_test_data.json"):
    ...
    def load_test_data(self):
        ...
        try:
            ...
            self.log_status(f"Loaded {len(self.test_data)} progressive messages from {self.test_file}")
        except FileNotFoundError:
            self.log_status(f"ERROR: Test data file not found at {self.test_file}!")
            self.test_data = []
        except Exception as e:
            self.log_status(f"ERROR loading test data from {self.test_file}: {e}")
            self.test_data = []
