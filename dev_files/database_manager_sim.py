...
class DBManager:
    def __init__(self, db_path="blackjack_data_sim.db"):
        """Initializes the database connection and creates tables if they don't exist."""
        self.db_path = db_path
        self.conn = None
        try:
            self.conn = sqlite3.connect(self.db_path, check_same_thread=False)
            print(f"[SIM] Successfully connected to database at {self.db_path}")
            self.create_tables()
        except sqlite3.Error as e:
            print(f"[SIM] Error connecting to database: {e}")
    ...
    def create_tables(self):
        ...
        self.conn.commit()
        print("[SIM] Enhanced database schema created successfully with analytics tables.")
