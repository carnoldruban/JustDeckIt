...
if TYPE_CHECKING:
    from blackjack_tracker.core_logic.cards import Card, Shoe, create_shoe_cards
...
class Zone:
    ...
    def get_summary(self) -> str:
        ...
        for category_key in ordered_categories:
            ...
            elif category_key == CATEGORY_NEUTRAL: category_display_name = "Neutrals"
...
class ShuffleManager:
    ...
    def create_fresh_shoe_cards(num_decks: int) -> List['Card']:
        ...
        # The import is structured to work whether the application is run as a script or a module.
        try:
    ...
    def deal_card_from_current_zone(self, specific_card: Optional['Card'] = None) -> Optional['Card']:
        ...
        if dealt_card_from_zone:
            try:
                self.shoe.undealt_cards.remove(dealt_card_from_zone) # Ensure consistency with main shoe
            except ValueError:
                # This warning indicates a potential desync if the card was in zone but not shoe.undealt_cards
    ...
    def get_all_zone_summaries(self) -> List[str]:
        if not self.zones: return ["Zone tracking is not active or no zones defined."]
    ...
    def riffle_card_lists(list1: List['Card'], list2: List['Card'], imperfection: float = 0.05) -> List['Card']:
        ...
        while deck1 or deck2:
            ...
            if random.random() < imperfection and deck1: clump1_size = random.randint(2, 4)
            for _ in range(clump1_size):
                if deck1: result.append(deck1.pop(0))
                else: break
            clump2_size = random.randint(1, 3)
            if random.random() < imperfection and deck2: clump2_size = random.randint(2, 4)
    ...
    def strip_shuffle_pile(pile_to_strip: List['Card'], strip_packet_size_input: Optional[int]) -> List['Card']:
        ...
        Small packets are taken from the top of the input pile and placed on top of a new pile.
        This process effectively reverses the order of the packets.

        Args:
            pile_to_strip (List['Card']): The list of cards to be strip shuffled.
            strip_packet_size_input (Optional[int]): The desired packet size for stripping.
                If None, 0, or invalid, a default size (approx. 1/7th of the pile) will be used.
        ...
        if strip_packet_size_input and strip_packet_size_input > 0:
            ...
            # Default packet size: aim for roughly 7 strips, ensure at least 1 card per packet.
            packet_size = max(1, len(working_pile) // 7)
            if packet_size == 0 and len(working_pile) > 0 : # Handles very small piles len < 7
                packet_size = 1
        ...
        # The way a strip shuffle is described (taking from top of source, placing on top of new pile)
        # means the first packet taken ends up at the bottom of the result.
        # So, we reverse the order of the packets themselves before concatenating.
...
if __name__ == '__main__':
    try:
        ...
        class Card:
            ...
            def __eq__(self, other): return isinstance(other, Card) and self.suit == other.suit and self.rank_str == other.rank_str
